...
class RbacActionColumn extends Column {
    ...
    public function init() {
        ...
        $this->checkTemplateBYUser($user, $controller);
     ...
     * @inheritdoc
     */
    protected function checkTemplateBYUser($user, $controller) {
        $actions_arr = ['view', 'update', 'delete'];
        $all_privileges_link = '/' . ltrim($controller->id) . '/*';
        foreach ($actions_arr as $col_action) {
            if (!Yii::$app->user->can($all_privileges_link) && !Yii::$app->user->can('/' . ltrim($controller->id) . "/$col_action")) {
                //print '{'.$col_action.'}';
                $this->template = str_replace('{'.$col_action.'}','', $this->template);
            }
        }
    }
